package framework.animation.annotation

/*
 * Copyright (C) 2024 The Android Open Source Project BY GUTIERRE GUIMARAES
 *
...
@java.lang.annotation.Target(
 ...
 annotation class RestrictTo(
    ...
    enum class Scope {
        ...
        /**
         * Restrict usage to code within packages whose groups share the same library group prefix
         * up to the last ".", so for example libraries foo.bar:lib1 and foo.baz:lib2 share the
         * prefix "foo." and so they can use each other's apis that are restricted to this scope.
         * Similarly for com.foo.bar:lib1 and com.foo.baz:lib2 where they share "com.foo.". Library
         * com.bar.qux:lib3 however will not be able to use the restricted api because it only
         * shares the prefix "com." and not all the way until the last ".".
         */
        LIBRARY_GROUP_PREFIX,

        /**
         * Restrict usage to code within the same group ID (based on gradle group ID). This is an
         * alias for [LIBRARY_GROUP_PREFIX].
         *
